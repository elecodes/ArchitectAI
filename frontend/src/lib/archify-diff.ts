import type { ArchifyDiagramIR, ArchifyNode, ArchifyEdge } from './archify';
...
export function compareArchifyIR(
  ...
  for (const [id, afterNode] of afterNodesMap.entries()) {
    ...
    if (!beforeNode) {
      nodeChanges.push({ changeType: 'added', node: afterNode });
    } else {
      const details: string[] = [];
      if (beforeNode.role !== afterNode.role) details.push(`Role changed from ${beforeNode.role} to ${afterNode.role}`);
      if (beforeNode.layer !== afterNode.layer) details.push(`Layer changed from ${beforeNode.layer} to ${afterNode.layer}`);

      if (details.length > 0) {
        nodeChanges.push({ changeType: 'modified', node: afterNode, previousNode: beforeNode, details });
      } else {
        nodeChanges.push({ changeType: 'unchanged', node: afterNode });
      }
    }
  }
  ...
  for (const [id, beforeNode] of beforeNodesMap.entries()) {
    if (!afterNodesMap.has(id)) {
      nodeChanges.push({ changeType: 'removed', node: beforeNode });
    }
  }
  ...
  for (const [key, afterEdge] of afterEdgesMap.entries()) {
    if (!beforeEdgesMap.has(key)) {
      edgeChanges.push({ changeType: 'added', edge: afterEdge });
    } else {
      edgeChanges.push({ changeType: 'unchanged', edge: afterEdge });
    }
  }

  for (const [key, beforeEdge] of beforeEdgesMap.entries()) {
    if (!afterEdgesMap.has(key)) {
      edgeChanges.push({ changeType: 'removed', edge: beforeEdge });
    }
  }
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
    ...
    .metrics-bar { display: flex; gap: 1.5rem; margin-bottom: 1.5rem; }
    .metric-card { background: var(--panel); border: 1px solid var(--border); border-radius: 8px; padding: 1rem; flex: 1; text-align: center; }
    ...
    .grid-container { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 1.25rem; }
    .change-card { background-color: var(--panel); border: 1px solid var(--border); border-radius: 10px; padding: 1.25rem; position: relative; }
    ...
    .change-tag { display: inline-block; font-size: 0.7rem; font-weight: 700; text-transform: uppercase; padding: 0.2rem 0.5rem; border-radius: 4px; margin-bottom: 0.5rem; }
...
<body class="dark">
  <header>
    <div class="title-group">
      <h1>${diff.title}</h1>
      <p>Comparing ${diff.before.meta.title} ➔ ${diff.after.meta.title}</p>
    </div>
    <div class="tabs">
  ...
  <main>
    <div id="pane-delta" class="view-pane active">
      <div class="metrics-bar">
        <div class="metric-card"><p>Nodes Added</p><div class="metric-num metric-added">+${diff.summary.nodesAdded}</div></div>
        <div class="metric-card"><p>Nodes Removed</p><div class="metric-num metric-removed">-${diff.summary.nodesRemoved}</div></div>
        <div class="metric-card"><p>Nodes Modified</p><div class="metric-num metric-modified">~${diff.summary.nodesModified}</div></div>
      </div>
      ...
      <div class="grid-container">
        ${diff.nodeChanges
          ...
          <div class="change-card ${c.changeType}">
            ...
            <div style="font-weight: 600; font-size: 1.05rem;">${c.node.label}</div>
    ...
    <div id="pane-before" class="view-pane">
      <div class="grid-container">
        ${diff.before.nodes.map((n) => `<div class="change-card"><span class="change-tag tag-unchanged">${n.role}</span><div>${n.label}</div></div>`).join('')}
      </div>
    </div>

    <div id="pane-after" class="view-pane">
      <div class="grid-container">
        ${diff.after.nodes.map((n) => `<div class="change-card"><span class="change-tag tag-unchanged">${n.role}</span><div>${n.label}</div></div>`).join('')}
      </div>
    </div>
  </main>
