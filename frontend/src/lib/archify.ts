...
export function generateArchifyIR(
  ...
  const dependencyGraph: any[] = Array.isArray(arch?.dependencyGraph) ? arch.dependencyGraph : [];
  const boundedContexts: any[] = Array.isArray(arch?.boundedContexts) ? arch.boundedContexts : [];

  const nodes: ArchifyNode[] = components.map((comp) => {
    let role: ArchifyNode['role'] = (comp.layer as ArchifyNode['role']) || 'application';
    if (comp.layer === 'infrastructure' && String(comp.name).toLowerCase().includes('data')) {
      role = 'database';
    }
  ...
  const clusters: ArchifyCluster[] = boundedContexts.map((bc) => {
    ...
    const aggregates: string[] = Array.isArray(bc.aggregates) ? bc.aggregates : [];
    const clusterNodes = nodes
      .filter(
        (node) =>
          aggregates.some((agg) => node.label.toLowerCase().includes(String(agg).toLowerCase())) ||
          node.label.toLowerCase().includes(String(bc.name || '').toLowerCase())
      )
      .map((n) => n.id);

    return {
      id: clusterId,
      name: bc.name || 'Bounded Context',
      nodes: clusterNodes.length > 0 ? clusterNodes : nodes.slice(0, 2).map((n) => n.id),
      responsibilities: Array.isArray(bc.responsibilities) ? bc.responsibilities : [],
    };
  });
