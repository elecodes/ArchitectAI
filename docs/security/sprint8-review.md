...
| S3 — Dependabot not configured | MEDIUM | **FIXED** | `.github/dependabot.yml` (github-actions + npm root + npm `frontend/`, weekly) | Root and `frontend/` npm workspaces covered (frontend entry added in wave 5) |
| S4 — IDOR on artifacts | MEDIUM (→HIGH multi-user) | **FIXED** | `src/db/repositories/artifact-repo.ts:42-51` (`getArtifact` JOIN `p.owner_id`), `:53-67` (`listArtifacts` JOIN) | All call sites pass `req.userId`: `generation.ts:198,268,341,401,407,472`, `artifacts.ts:11`, `export.ts:27`, `feedback.ts` (scoped `getArtifact` before upsert — wave 5) |
...
| R7 | ~~Dependabot does not cover `frontend/` npm workspace~~ | LOW | **RESOLVED** in wave 5 — `.github/dependabot.yml` now has a `frontend/` npm entry | Future workspaces need their own dependabot entry |
...
- [x] **Dependabot PRs triaged** — weekly npm (root + `frontend/`) and github-actions updates (R7)
