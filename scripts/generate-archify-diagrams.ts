import { generateArchifyIR, compileArchifyHtml, type ArchifyDiagramIR } from '../src/diagrams/archify.js';
import type { ArchitectureDocument } from '../src/generation/schemas.js';
import { writeFileSync, mkdirSync } from 'node:fs';
import { join } from 'node:path';

/**
 * Archify Diagram Automation Script
 *
 * Automatically compiles Archify system architecture diagrams, adds explicit creation timestamp tracking
 * (meta.createdAt), and emits both JSON IR and self-contained interactive HTML diagrams with download capabilities.
 */
...
const runtimeIRTemplate: Omit<ArchifyDiagramIR, 'meta'> & { meta: Omit<ArchifyDiagramIR['meta'], 'createdAt'> } = {
  version: '2.17.0',
  meta: {
...
export function automateArchifyGeneration(outDir?: string): void {
  const createdAt = new Date().toISOString();
  console.log(`[Archify Automation] Compiling system diagrams at timestamp: ${createdAt}`);

  const targetDir = outDir || join(process.cwd(), 'docs', 'architecture');
  mkdirSync(targetDir, { recursive: true });

  // 1. System Map
  const systemMapIR = generateArchifyIR(
    architectAiDoc,
    'ArchitectAI v1.7.0 Architecture System Map',
    { createdAt }
  );
  const systemMapHtml = compileArchifyHtml(systemMapIR);
  writeFileSync(join(targetDir, 'system-map.json'), JSON.stringify(systemMapIR, null, 2), 'utf-8');
  writeFileSync(join(targetDir, 'system-map.html'), systemMapHtml, 'utf-8');
  console.log(` ✓ System Map JSON IR saved to: ${join(targetDir, 'system-map.json')}`);
  console.log(` ✓ System Map Interactive HTML saved to: ${join(targetDir, 'system-map.html')}`);

  // 2. Runtime Architecture
  const runtimeIR: ArchifyDiagramIR = {
    ...runtimeIRTemplate,
    meta: {
      ...runtimeIRTemplate.meta,
      createdAt,
    },
  };
  const runtimeHtml = compileArchifyHtml(runtimeIR);
  writeFileSync(join(targetDir, 'runtime-architecture.json'), JSON.stringify(runtimeIR, null, 2), 'utf-8');
  writeFileSync(join(targetDir, 'runtime-architecture.html'), runtimeHtml, 'utf-8');
  console.log(` ✓ Runtime Architecture JSON IR saved to: ${join(targetDir, 'runtime-architecture.json')}`);
  console.log(` ✓ Runtime Architecture Interactive HTML saved to: ${join(targetDir, 'runtime-architecture.html')}`);
}

automateArchifyGeneration();
